...
INPUT_DIR = ROOT / "data/processed/super_csvs/research_ready_plus/clubelo_understat_transfermarkt"
REPORT_DIR = ROOT / "outputs/reports/transfermarkt_predictive"

MARKETS = {
    "btts": {
        "file": "super_btts_footiqo_top5_clubelo_understat_transfermarkt_research_v1.csv",
        "target": "target_btts_yes",
        "baseline": "btts_yes_no_vig_prob",
    },
    "ou25": {
        "file": "super_ou25_footiqo_top5_clubelo_understat_transfermarkt_research_v1.csv",
        "target": "target_over_2_5",
        "baseline": "ou25_over_no_vig_prob",
    },
    "ou35": {
        "file": "super_ou35_footiqo_top5_clubelo_understat_transfermarkt_research_v1.csv",
...
def footiqo_goals_form_cols(df: pd.DataFrame) -> list[str]:
    ...
    return [
        ...
        and not c.startswith(("home_tm_", "away_tm_", "tm_"))
...
def transfermarkt_core_cols(df: pd.DataFrame) -> list[str]:
    ...
    for c in numeric_cols(df):
        ...
        if c.startswith(("home_tm_", "away_tm_", "tm_")):
            if c not in {"home_tm_club_id", "away_tm_club_id"}:
                out.append(c)
    return out


def transfermarkt_value_diff_cols(df: pd.DataFrame) -> list[str]:
    return [
        c
        for c in numeric_cols(df)
        if c.startswith("tm_")
        and c.endswith("_diff")
...
def feature_groups(df: pd.DataFrame, target: str, baseline: str) -> dict[str, list[str]]:
    ...
    tm_core = transfermarkt_core_cols(df)
    tm_diff = transfermarkt_value_diff_cols(df)
    ...
    groups = {
        ...
        "market_plus_transfermarkt_core": [baseline] + tm_core,
        "market_plus_transfermarkt_value_diff": [baseline] + tm_diff,
        "market_plus_transfermarkt_plus_clubelo": [baseline] + tm_core + clubelo,
        "market_plus_transfermarkt_plus_understat_core": [baseline] + tm_core + under,
        "market_plus_transfermarkt_plus_understat_core_plus_clubelo": [baseline] + tm_core + under + clubelo,
        "market_plus_footiqo_rolling_goals_form": [baseline] + goals,
        "market_plus_footiqo_rolling_goals_form_plus_transfermarkt": [baseline] + goals + tm_core,
        "market_plus_footiqo_rolling_goals_form_plus_transfermarkt_plus_understat_core": [baseline] + goals + tm_core + under,
        "market_plus_all_safe_light": [baseline] + goals + tm_core + under + clubelo + league + light_flags,
    }
...
def models_for(group_name: str) -> dict[str, object]:
    ...
    xgb_groups = {
        ...
        "market_plus_transfermarkt_plus_clubelo",
        "market_plus_transfermarkt_plus_understat_core",
        "market_plus_footiqo_rolling_goals_form_plus_transfermarkt",
    }
...
def evaluate_market(market: str, spec: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    df["transfermarkt_stale_flag"] = (
        (pd.to_numeric(df["home_tm_latest_valuation_days_ago"], errors="coerce") > 365)
        | (pd.to_numeric(df["away_tm_latest_valuation_days_ago"], errors="coerce") > 365)
    )
            ...
            pd.DataFrame(
                {
                    ...
                    "p": base_prob,
                    "p_market": base_prob,
                    "transfermarkt_stale": test["transfermarkt_stale_flag"].astype(bool).values,
                }
            )
        )
...
def leakage_row(market: str, df: pd.DataFrame, groups: dict[str, list[str]]) -> dict[str, object]:
    ...
    source_ids_excluded = not any("source_match_id" in c or c.endswith("_alias_id") or c.endswith("_club_id") for c in used)
    ...
    rejected_alias_not_used = True
    no_future = True
    no_current_club = not any("current_club" in c for c in df.columns)
    no_current_value = not any("current_value" in c for c in df.columns)
    no_lineups = not any("lineup" in c.lower() for c in df.columns)
    no_appearances = not any("appearance" in c.lower() for c in df.columns)
    row = {
        "market": market,
        ...
        "no_future_valuations_or_transfers_used": no_future,
        "current_club_fields_excluded": no_current_club,
        "current_value_fields_excluded": no_current_value,
        "game_lineups_excluded": no_lineups,
        "same_match_appearances_excluded": no_appearances,
        "odds_timing_unknown": bool(df["odds_timing_flag"].eq("unknown").all()),
        "transfermarkt_staleness_documented": "home_tm_latest_valuation_days_ago" in df.columns and "away_tm_latest_valuation_days_ago" in df.columns,
...
def staleness_diagnostics(preds: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for keys, g in preds.groupby(["market", "feature_group", "model", "transfermarkt_stale"], dropna=False):
        m = metric_dict(g["y"], g["p"])
        bm = metric_dict(g["y"], g["p_market"])
        rows.append(
            {
                ...
                "feature_group": keys[1],
                "model": keys[2],
                "transfermarkt_stale": bool(keys[3]),
                "n": len(g),
                **m,
                "market_log_loss": bm["log_loss"],
                "market_brier": bm["brier"],
...
def previous_reports() -> pd.DataFrame:
    candidates = [
        ...
        ("understat", ROOT / "outputs/reports/understat_predictive/understat_by_market_model.csv"),
        ("understat", ROOT / "outputs/reports/understat_predictive/understat_controlled_summary.csv"),
...
def decide(summary: pd.DataFrame, league: pd.DataFrame, leakage: pd.DataFrame) -> tuple[str, pd.DataFrame]:
    ...
    if (ddf["market_decision"] == "ready_for_value_diagnostic_research_only").any() and leakage_pass:
        decision = "transfermarkt_predictive_ready_for_value_diagnostic_research_only"
    elif (ddf["market_decision"] == "market_recalibration_only").any():
        decision = "transfermarkt_predictive_market_recalibration_only"
    else:
        decision = "transfermarkt_predictive_rejected_no_gain"
    return decision, ddf
...
def write_markdown(decision: str, decision_df: pd.DataFrame, summary: pd.DataFrame, prev: pd.DataFrame) -> None:
    ...
    lines = [
        "# Transfermarkt Controlled Predictive Audit",
        "",
        f"Decision: **{decision}**",
        "",
        ...
        "- Transfermarkt valuation staleness is tracked through latest valuation days-ago fields.",
        "- Predictive improvement, if any, is not a betting edge.",
        "- No confirmed edge is claimed.",
    ]
    (REPORT_DIR / "transfermarkt_controlled_predictive_audit.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    decision_lines = [
        "# Transfermarkt Predictive Decision",
        "",
        f"Decision: **{decision}**",
        "",
    ...
    (REPORT_DIR / "transfermarkt_predictive_decision.md").write_text("\n".join(decision_lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    summary_out.to_csv(REPORT_DIR / "transfermarkt_controlled_summary.csv", index=False)
    summary.to_csv(REPORT_DIR / "transfermarkt_by_market_model.csv", index=False)
    folds.to_csv(REPORT_DIR / "transfermarkt_by_market_season.csv", index=False)
    league.to_csv(REPORT_DIR / "transfermarkt_by_market_league.csv", index=False)
    lolo.to_csv(REPORT_DIR / "transfermarkt_leave_one_league_out.csv", index=False)
    fg.to_csv(REPORT_DIR / "transfermarkt_feature_group_comparison.csv", index=False)
    stale.to_csv(REPORT_DIR / "transfermarkt_staleness_diagnostics.csv", index=False)
    leakage.to_csv(REPORT_DIR / "transfermarkt_leakage_checks.csv", index=False)
    prev.to_csv(REPORT_DIR / "transfermarkt_previous_report_comparison.csv", index=False)
