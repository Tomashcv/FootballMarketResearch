...
DATA_PATH = Path("data/processed/features/football_feature_matrix_v1.csv")
...
REPORT_PATH = Path("outputs/reports/feature_matrix_v1_ah_predictive_audit.md")
SUMMARY_PATH = Path("outputs/reports/feature_matrix_v1_ah_predictive_summary.csv")
BUCKET_PATH = Path("outputs/reports/feature_matrix_v1_ah_edge_bucket_calibration.csv")
NEGATIVE_PATH = Path("outputs/reports/feature_matrix_v1_ah_negative_controls.csv")
ROBUSTNESS_PATH = Path("outputs/reports/feature_matrix_v1_ah_robustness.csv")
VALUE_REPORT_PATH = Path("outputs/reports/feature_matrix_v1_ah_value_review.md")
VALUE_FIXED_PATH = Path("outputs/reports/feature_matrix_v1_ah_value_fixed_rules.csv")
VALUE_NESTED_PATH = Path("outputs/reports/feature_matrix_v1_ah_value_nested_selection.csv")
VALUE_CONTROLS_PATH = Path("outputs/reports/feature_matrix_v1_ah_value_controls.csv")
VALUE_ROBUSTNESS_PATH = Path("outputs/reports/feature_matrix_v1_ah_value_robustness.csv")
...
def load_data() -> pd.DataFrame:
    ...
    data["AvgAHH"] = pd.to_numeric(data["ah_avg_odds_home"], errors="coerce")
    data["AvgAHA"] = pd.to_numeric(data["ah_avg_odds_away"], errors="coerce")
    data["no_vig_ah_home_probability"] = pd.to_numeric(data["ah_avg_prob_home"], errors="coerce")
    data["no_vig_ah_away_probability"] = pd.to_numeric(data["ah_avg_prob_away"], errors="coerce")
    ...
    data["home_profit"] = np.where(target.eq(1), data["AvgAHH"] - 1.0, np.where(target.eq(0), -1.0, np.nan))
    data["away_profit"] = np.where(target.eq(0), data["AvgAHA"] - 1.0, np.where(target.eq(1), -1.0, np.nan))
    data["home_label"] = np.where(target.eq(1), "win", np.where(target.eq(0), "loss", "push_or_missing"))
    data["away_label"] = np.where(target.eq(0), "win", np.where(target.eq(1), "loss", "push_or_missing"))
    required = ["match_date", "league", "season_end_year", TARGET, "AvgAHH", "AvgAHA", "market_home_probability", "market_away_probability"]
...
def feature_groups(data: pd.DataFrame) -> dict[str, list[str]]:
    ...
    baseline = [
        ...
        "ah_avg_market_overround",
    ]
    baseline = [c for c in baseline if c in data.columns]
    odds_board = sorted(c for c in (by_group.get("odds_board_features", set()) & safe) if c.startswith("ah_"))
    ...
    full_safe = sorted(safe | {"AvgAHH", "AvgAHA", "no_vig_ah_home_probability", "no_vig_ah_away_probability"})
    return {
        "ah_market_baseline_only": baseline,
        "ah_market_plus_odds_board": sorted(set(baseline) | set(odds_board)),
        "ah_market_plus_form_elo": sorted(set(baseline) | set(form_elo)),
        "ah_feature_matrix_v1_full_safe": full_safe,
    }
...
def fit_model(train: pd.DataFrame, validation: pd.DataFrame, test: pd.DataFrame, model: str, features: list[str], seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    if model == "logistic_elasticnet":
        pipe = Pipeline([("impute", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("model", SGDClassifier(loss="log_loss", penalty="elasticnet", alpha=0.0005, l1_ratio=0.2, max_iter=120, tol=1e-3, random_state=seed, average=True))])
        pipe.fit(train[features], train[TARGET].astype(int))
        cls = list(pipe.named_steps["model"].classes_).index(1)
        return pipe.predict_proba(validation[features])[:, cls], pipe.predict_proba(test[features])[:, cls]
    ...
    for control in controls:
        ...
        for year in TEST_YEARS:
            ...
            if control == "shuffled_train_labels":
                ...
                replace = [c for c in features if c not in {"AvgAHH", "AvgAHA", "no_vig_ah_home_probability", "no_vig_ah_away_probability", "ah_avg_market_overround"}]
...
def advancement_candidates(summary: pd.DataFrame, buckets: pd.DataFrame, robustness: pd.DataFrame, negative: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in summary.iterrows():
        if row["model"] in {"raw_market_baseline", "xgboost_shallow"}:
            continue
        ...
        beats_prior = (row["mean_delta_log_loss_vs_raw_market"] < PRIOR_LOGLOSS_DELTA and row["mean_delta_brier_vs_raw_market"] < PRIOR_BRIER_DELTA) or (
            row["mean_delta_log_loss_vs_raw_market"] < 0 and row["mean_delta_brier_vs_raw_market"] < 0 and high_sane and row["mean_delta_ece_vs_raw_market"] <= 0
        )
        passes = (
        ...
        rows.append({**row.to_dict(), "advancement_gate_passed": bool(passes), "beats_or_justifies_prior_benchmark": bool(beats_prior), "high_edge_sane": bool(high_sane), "negative_controls_failed": bool(controls_fail), "robustness_best_exclusions_positive": bool(best_ok)})
    return pd.DataFrame(rows)
...
def write_reports(summary, candidates, value_fixed, value_nested, final_classification, data, groups):
    ...
    lines = [
        ...
        "",
        "AH line note: `AHh` is not present in feature_matrix_v1, so it was not modeled or reconstructed. Value settlement columns use the matrix settlement-derived target and average AH odds only.",
        "",
        ...
        "## Advancement Gate",
        "",
        markdown_table(gate, ["model", "feature_group", "advancement_gate_passed", "beats_or_justifies_prior_benchmark", "mean_delta_log_loss_vs_raw_market", "mean_delta_brier_vs_raw_market", "mean_delta_ece_vs_raw_market", "improved_years", "high_edge_sane", "negative_controls_failed", "robustness_best_exclusions_positive"], 60),
        "",
    ...
    value_lines = [
        "# feature_matrix_v1 AH Value Review",
        "",
        f"Final classification: `{final_classification}`",
        "",
...
def main() -> None:
    ...
    if prelim.empty:
        prelim = overall[~overall["model"].eq("raw_market_baseline")].sort_values("mean_delta_log_loss_vs_raw_market").head(2).copy()
    negative_parts = []
    robustness_parts = []
